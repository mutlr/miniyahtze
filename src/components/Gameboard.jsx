...
const Gameboard = ({ route }) => {
    ...
    useEffect(() => {
        saveGame().then(() => console.info("Game saved!")).catch(err => console.error(err))
    }, [gameOver])
    ...
    const pickPoints = (value) => {
        ...
        if (gameOver | !checkIfMovesLeft()) {
            ...
            setError('You have already used that number')
            return
        }
    ...
    const saveGame = async () => {
        try {
            ...
            await AsyncStorage.setItem(SCOREBOARD_KEY, JSON.stringify(result));
        } catch (error) {
            console.error(`Error adding game result to storage`)
        }
    }
